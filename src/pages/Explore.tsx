...
import { Search, Hash, Share2 } from 'lucide-react';
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filteredReels.map(reel => (
            ...
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-transparent to-transparent opacity-80 group-hover:opacity-100 transition z-10"></div>
              
              <div className="absolute bottom-0 left-0 right-0 p-3 flex items-center justify-between z-20">
                <div className="flex items-center gap-2 cursor-pointer flex-1 min-w-0" onClick={(e) => { e.stopPropagation(); navigate(`/profile/${reel.userId}`); }}>
                  <div className="w-6 h-6 rounded-full bg-surface overflow-hidden border border-border/50 flex items-center justify-center text-[10px] font-bold text-foreground flex-shrink-0">
                    {reel.userPhotoURL ? <img src={reel.userPhotoURL} alt="User" className="w-full h-full object-cover" /> : reel.username?.[0].toUpperCase()}
                  </div>
                  <span className="text-white text-xs font-semibold truncate drop-shadow-md hover:underline">@{reel.username}</span>
                </div>
